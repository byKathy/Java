...
Java的volatile关键字是用来标记一个Java变量作为“被存储在主内存”。更确切地说，这意味着，一个volatile变量的每一个读操作将在电脑的主内存中读取，而不是从CPU高速缓存和每一个写入volatile变量将被写入主内存，而不是仅仅在CPU缓存。 

事实上，从Java5 volatile关键字保证远远不止这些volatile变量写入，并从主内存中读取。我将解释在下面的章节。
